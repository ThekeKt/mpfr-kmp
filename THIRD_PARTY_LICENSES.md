...
Distribution pattern: `libmpfr`/`libgmp` are built reproducibly from the pinned
sources under `third_party/mpfr/` and are consumed as dynamically loaded shared
libraries on every target; the compliance summary below records the obligations.
